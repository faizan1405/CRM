  ...
  return (
    <div className="flex flex-col gap-3">
      {priorities.length === 0 ? (
        ...
        priorities.map((item) => (
          <ActionCard key={item.id} href={`/leads?selected=${encodeURIComponent(item.leadId)}`} aria-label={`Open lead ${item.leadName}`} className="group flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-xl border bg-white p-4 shadow-sm transition-all hover:border-slate-300 hover:shadow-md">
