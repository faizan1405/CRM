...
export function RevenueTrend({ data }: { data: RevenueTrendPoint[] }) {
  ...
  return <AnalyticsCard title="Won revenue trend" description="Revenue recognized from closed-won deals." action={<span className="hidden rounded-lg bg-green-50 px-2.5 py-1 text-xs font-semibold text-green-700 sm:block">{compactNumber.format(total)} total</span>} className="xl:col-span-2">
    {data.length === 0 || data.every((point) => point.revenue === 0) ? <ChartEmpty label="won revenue" /> : <>
      <div className="h-64 w-full" aria-hidden="true"><ResponsiveContainer width="100%" height="100%"><AreaChart data={data} margin={{ top: 8, right: 4, left: -18, bottom: 0 }}>
        ...
        <Tooltip formatter={(value) => [currency.format(Number(value)), "Won revenue"]} labelFormatter={(label) => `Period: ${label}`} contentStyle={{ borderRadius: 12, borderColor: "#e2e8f0", boxShadow: "0 8px 24px rgba(15,23,42,.1)" }} />
      ...
      <p className="sr-only">Won revenue totals {currency.format(total)} across {data.length} periods. {peak ? `Highest period was ${peak.label} at ${currency.format(peak.revenue)}.` : ""}</p>
