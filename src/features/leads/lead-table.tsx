    ...
    <div className="hidden overflow-x-auto lg:block">
      <div role="table" className="w-full min-w-[1060px] text-left">
        <div role="rowgroup">
          <div
            role="row"
            className={`border-b border-slate-200 bg-slate-50/80 text-xs font-semibold uppercase tracking-wide text-slate-500 ${LEAD_TABLE_GRID}`}
          >
        ...
        <div role="rowgroup" className="divide-y divide-slate-100">
          {leads.map((lead) => {
            ...
            return (
              ...
              >
                <div role="cell" className={`px-5 py-4 ${theme.leftBorder}`}>
                  <div className="flex items-center gap-1.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 text-sm text-[var(--muted)] truncate">{lead.business || "No business added"}</p>
                    </div>
                  </div>
                </div>
                <div role="cell" className="whitespace-nowrap px-4 py-4">
                ...
                <div role="cell" className="whitespace-nowrap px-4 py-4 text-sm text-slate-700">
                  {lead.phone || "—"}
                </div>
                ...
                <div role="cell" className="whitespace-nowrap px-4 py-4 text-sm font-medium text-slate-700">
                  {formatCurrency(lead.quotedAmount)}
                </div>
                <div role="cell" className="px-5 py-4 text-right">
                  <div className="inline-flex items-center gap-1 justify-end">
                    <button
                      type="button"
                      onClick={() => onDelete(lead)}
                      aria-label={`Delete ${lead.name}`}
                      className="grid size-10 place-items-center rounded-lg text-slate-500 hover:bg-rose-50 hover:text-rose-700 focus-visible:ring-2 focus-visible:ring-rose-600"
                    >
                      <Trash2 size={17} />
                    </button>
                    <a
                      href={getTelephoneHref(lead.phone)}
                      className="grid size-10 place-items-center rounded-lg text-slate-500 transition-colors hover:bg-blue-50 hover:text-blue-700"
                    ...
                    <button
                      ...
                      className="grid size-10 place-items-center rounded-lg text-slate-500 transition-colors hover:bg-emerald-50 hover:text-emerald-700"
                    ...
                    <button
                      ...
                      className="inline-flex min-h-10 items-center gap-1 rounded-lg px-3 text-sm font-semibold text-blue-700 transition-colors hover:bg-blue-50 cursor-pointer"
