        ...
        <tbody className="divide-y divide-slate-100">
          {leads.map((lead) => (
            <tr key={lead.id} className="bg-white transition-colors hover:bg-slate-50/70">
              ...
              <td className="px-4 py-4 text-sm text-slate-700">{lead.source || "—"}</td>
              ...
              <td className="px-5 py-4 text-right">
                <button type="button" onClick={() => onSelect(lead)} className="inline-flex min-h-10 items-center gap-1 rounded-lg px-3 text-sm font-semibold text-blue-700 hover:bg-blue-50" aria-label={`View ${lead.name}`}>
