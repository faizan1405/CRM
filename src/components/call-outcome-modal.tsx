...
export function CallOutcomeModal({ isOpen, lead, onClose, onFollowUpScheduled, onStatusChanged }: Props) {
  ...
  const primary = "min-h-11 rounded-xl bg-blue-600 px-4 text-sm font-semibold text-white disabled:opacity-50";
  const secondary = "min-h-11 rounded-xl border border-slate-200 px-4 text-sm font-semibold text-slate-700 disabled:opacity-50";
  return <BottomSheet isOpen={isOpen} onClose={onClose} title={outcome ? `Call Outcome: ${outcome === "NOT_PICKED" ? "Not Picked" : outcome === "CALL_BACK" ? "Call Back" : outcome === "PICKED" ? "Picked" : "Interested"}` : "Call Outcome"} saving={saving}>
    {!outcome ? <div className="grid grid-cols-2 gap-3">{([["PICKED", "Picked"], ["NOT_PICKED", "Not Picked"], ["CALL_BACK", "Call Back"], ["INTERESTED", "Interested"]] as const).map(([value, label]) => <button key={value} type="button" onClick={() => select(value)} className={secondary}>{label}</button>)}</div> : <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 pt-2">{outcome === "PICKED" ? <button type="button" onClick={() => save("none")} disabled={saving} className={primary}>Save Call Outcome</button> : <>
        {outcome !== "CALL_BACK" && <button type="button" onClick={() => save("none")} disabled={saving} className={secondary}>Save Call Outcome</button>}
        {existing ? <><button type="button" onClick={() => save("keep")} disabled={saving} className={secondary}>Keep Existing</button><button type="button" onClick={() => save("replace")} disabled={saving || !date || !time} className={primary}>Replace/Reschedule</button></> : <button type="button" onClick={() => save("create")} disabled={saving || !date || !time} className={primary}>{outcome === "CALL_BACK" ? "Save Call Outcome & Callback" : "Save & Schedule Follow-up"}</button>}
      </>}</div>
    </div>}
  </BottomSheet>;
}
